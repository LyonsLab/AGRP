...
"""
%prog -i input_edges -o output_edges

Weighted maximum matching in general graphs.

`input_edges` and `output_edges` are simply three column files, sep by tabs.
node1    node2    weight
...
Code modified by Haibao, Apr-25-2014:
- Allow script to specify input file and outputfile.
"""
...
if __name__ == '__main__':
    ...
    for row in fp:
        atoms = row.split()
        a, b, w = [int(x) for x in atoms]
        edges.append((a, b, w))
    fp.close()

    start = time.clock()
    edges = maxWeightMatching(edges)

    fw = open(outfile, "w")
    for a, b, w in edges:
        print >> fw, "\t".join(str(x) for x in (a, b, w))
    totalweight = sum(x[-1] for x in edges)
